...
num_iteration = 3
latticeconst = (
    LATTICECONST[possible_elements[0]] * ratio[0]
    + LATTICECONST[possible_elements[1]] * ratio[1]
)
...
def make_samples(atomic_numbers, e_acts):
    ...
    for atom_num, e_acts in zip(atomic_numbers, e_acts):
        samples.append({"atomic_numbers": atom_num, "activation_energy": e_acts})

    # --- Normalize atomic numbers
    # Get the maximum number of atomic numbers
    max_of_max = np.max(
        np.array([np.max(samples[i]["atomic_numbers"]) for i in range(len(samples))])
    )
...
class AtomicDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        atomic_numbers_scaled = torch.tensor(
            self.samples[idx]["atomic_numbers_scaled"], dtype=torch.float32
        )
        energy = torch.tensor(
            self.samples[idx]["activation_energy"], dtype=torch.float32
        )
...
def train(netD, netG, optimD, optimG, n_epochs, dataloader, n_classes):
    ...
    for epoch in range(1, n_epochs + 1):
        for atomic_numbers_scaled, _, ranks in dataloader:
            ...
            pred_real = netD(
                atomic_numbers_scaled, ranks, n_classes=n_classes
            )  # 本物データ 判定
        ...
        if (epoch % 20) == 0:
            print(
                f"Epoch [{epoch:3d}/{n_epochs:3d}] | LossD: {lossD.item():.4f} | LossG: {lossG.item():.4f}"
            )
...
def generate_samples(netG, rank, n_generate=10, n_classes=0):
    ...
    # 0-1 -> atomic number
    fake_samples[fake_samples == 0] = ATOMIC_NUMBER[possible_elements[0]]
    fake_samples[fake_samples == 1] = ATOMIC_NUMBER[possible_elements[1]]
...
def train_and_generate(samples=None):
    ...
    train(
        netD, netG, optimD, optimG, n_epochs, dataloader=dataloader, n_classes=n_classes
    )

    # Generate fake-samples for "rank=1" surfaces, to generate the surfaces with lower activation energy.
    generated = generate_samples(
        netG, rank_to_generate, n_generate=n_generate, n_classes=n_classes
    )
...
if __name__ == "__main__":
    ...
    df = add_activation_energy_to_dataframe(
        df, iteration=0, e_acts=e_acts, formula_list=formula_list
    )

    samples = make_samples(
        atomic_numbers, e_acts
    )  # atomic numbers - descriptors, e_acts - target.
    samples = assign_rank(
        samples
    )  # Assign the smallest activation energy surfaces as "rank=0".
    generated = train_and_generate(
        samples=samples
    )  # train GAN and generate fake samples
    ...
    for i in range(num_iteration):
        ...
        df = add_activation_energy_to_dataframe(
            df, iteration=i + 1, e_acts=e_acts, formula_list=formula_list
        )

        newsamples = make_samples(
            atomic_numbers, e_acts
        )  # atomic numbers - descriptors, e_acts - target
        samples += newsamples
        samples = assign_rank(
